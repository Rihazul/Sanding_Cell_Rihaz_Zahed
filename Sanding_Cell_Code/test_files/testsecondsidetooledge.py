...
import yaml
from Server_Better_V2 import communicate,setup_logger,waitForBlending,turn_vibration_on,turn_vibration_off,putForce,releaseForce,moveOnlyJ6r,putForceYplus1,putForceXminus,putForceYminus1,putForceZplus,putForceXplus,putForce3Directionup
...
def main():
    ...
    point1pre=[0,-5,42,0,23.104,-90]
    point1=[0,1,42,0,23.104,-90]
    point12= [1,1,42,0,23.104,-90]
    point2=[500,1,42,0,23.104,-90]
    point2pre=[500,-5,42,0,23.104,-90]
    point2air=[530,-20,45,0,0,0]

    #Left Cycle 
    pointleft1pre=[517.991,10.699,42,0,25.104,0]
    pointleft1=[507.582,10.699,42,0,25.104,0]
    pointleft12=[507.582,11.699,42,0,25.104,0]
    pointleft2=[507.582,645.122,42,0,25.104,0]
    pointleft2pre=[517.991,645.122,42,0,25.104,0]
    pointleft2air=[521.172,667.503,15,0,0,90]

    #Top Cycle
    pointtop1pre=[500,658,42,0,23.104,90]
    pointtop1=[500,655,42,0,23.104,90]
    pointtop12=[499,655,42,0,23.104,90]
    pointtop2=[0,655,42,0,23.104,90]
    pointtop2pre=[0,658,42,0,23.104,90]
    pointtopair= [-35,693,4,0,23.104,-180]

    #Right Cycle
    topright1=[-3,645,42,0,23.104,-180]
    topright12=[-3,644,42,0,23.104,-180]
    topright1pre=[-6,645,42,0,23.104,-180]
    topright2=[-3,0,42,0,23.104,-180]
    topright2pre=[-6,0,42,0,23.104,-180]
    ...
    def perform_process_top(cps, config, points1):
        ...
        for point in points1:
            if point==point12:putForceZplus(cps=cps,force=10,tcp=config['coords']['tcpSideTool'],ucs=config['coords']['ucsTable2'],config=config)
            communicate(
                cps=cps,
                config=config,
                point=point,
    ...
    def perform_process_left(cps, config, points1):
        ...
        # Communicate to each point in points1
        for point in points1:
            if point==pointleft12:putForceZplus(cps=cps,force=10,tcp=config['coords']['tcpSideTool'],ucs=config['coords']['ucsTable2'],config=config)
            communicate(
                cps=cps,
                config=config,
                point=point,
    ...
    def perform_process_right(cps, config, points1):
        ...
        # Communicate to each point in points1
        for point in points1:
            if point==topright12:putForceZplus(
            cps=cps,
            force=5,
            tcp=config['coords']['tcpSideTool'],
            ucs=config['coords']['ucsTable2'],
