...
def main():
    ...
    zigzag_pathp,prepointp= generate_zigzag_path(x_coords=x_coords1, y_coords=y_coords1, z_coords=z_coords1, innerOffset=5,innerOffsetX=-10)
    print("zigzag_pathp=",zigzag_pathp)
    print("prepointp:", prepointp)
    #Second Pocket 2nd Cycle
    zigzag_pathp2,prepointp2= generate_zigzag_path(x_coords=x_coords1, y_coords=y_coords1, z_coords=z_coords1, innerOffset=5,innerOffsetX=-10)
    ...
    communicate(cps=cps,config=config,seventh=tcx,tcp=config['coords']['tcpReal'],ucs=config['coords']['ucsTable2'],speed=0.8,wait=True)
    communicate(cps=cps,config=config,point=spoint,tcp=config['coords']['tcpReal'],ucs=config['coords']['ucsTable2'],seventh=-1,speed=0.8,wait=True)
    # turn_vibration_on(cps)
    communicate(cps=cps,config=config,point=prepointp,tcp=config['coords']['tcpReal'],ucs=config['coords']['ucsTable2'],seventh=-1,speed=0.8,wait=True)
    #turn_vibration_on(cps)
