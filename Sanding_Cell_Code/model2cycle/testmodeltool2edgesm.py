...
def testmodel2tool2edgerun(force,cps,section=None,return_home=True,pass_index=None):
    def testmodel2tool2small(force,cps,section=None,return_home=True,pass_index=None):
        ...
        pointtop3=[-(p1[0]/2),p2[1]+8,p1[2]+45,0,22,90]
        ...
        if selected_section in (None, "top"):
            ...
            for cx in cx_points:
                ...
                perform_process_top(cps, config, points1=pointstopmain,force=force)
                communicate(cps=cps, config=config, point=pointtop2air, tcp=config['coords']['tcpSideTool'], ucs=config['coords']['ucsTable2'], seventh=-1, speed=speeed,velocity_profile="robot",wait=True)
            communicate(cps=cps,config=config,point=pointtopExtra,tcp=config['coords']['tcpSideTool'],ucs=config['coords']['ucsTable2'],seventh=-1,speed=speeed,velocity_profile="robot",wait=True)
    ...
    if xlen == "null":
        ...
        if xlen > 600:
            return tool2edgebig(force, cps, section=section, return_home=return_home,pass_index=pass_index)
        else:
            return testmodel2tool2small(force, cps, section=section, return_home=return_home,pass_index=pass_index)
    else:
...
if __name__ == "__main__":
    testmodel2tool2edgerun(force=3)
