...
def mod1tool2sidesrun(force,cps):
    def mod1tool2sidesbig(force,cps):
        ...
        #Last Tune
        communicate(cps=cps,config=config,seventh=-19,tcp=config['coords']['tcpSideTool'],ucs=config['coords']['ucsTable2'],speed=speeed,wait=False)
    ...
    if xlen == "null":
        ...
        if xlen > 600:
            mod1tool2sidesbig(force,cps)
        else:
            mod1tool2sidesmall(force,cps)
    else:
