...
def smalldoor1zizag(
    ...
    elif isinstance(ylen, (int, float)) and isinstance(xlen, (int, float)):  # Ensure it's numeric
        if ylen > 600 or xlen > 280:
            smalldoor1zizagbig(force,z,cps)
        else:
            smalldoor1zizagsmall(force,z,cps)
    else:
        print(f"Invalid ylen/xlen value type: {type(ylen)} / {type(xlen)} - expected number or 'null'")
