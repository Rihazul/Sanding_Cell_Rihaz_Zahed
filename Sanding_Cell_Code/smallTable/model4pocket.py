...
def smalldoor1pocket(force,z):
    def smalldoor1pocketsidebig(force,z):
    ...
    def smalldoor1pocketsidesmall(force,z):
        # Load configuration from YAML
        config = load_config()
    ...
    if ylen == "null":
        ...
        if ylen > 600:
            smalldoor1pocketsidebig(force,z)
        else:
            smalldoor1pocketsidesmall(force,z)
    else:
        print(f"Invalid ylen value type: {type(ylen)} - expected number or 'null'")


    def smalldoor4pocketsidebig(force,z):
            # Load configuration from YAML
            config = load_config()
            ...
            communicate(cps=cps,config=config,point=midhoming,tcp=config['coords']['tcptool1plane1'],ucs=config['coords']['ucsTable1'],seventh=-1,speed=0.7,wait=True)
            #communicate(cps=cps,config=config,seventh=conx1,tcp=config['coords']['tcptool1plane1'],ucs=config['coords']['ucsTable1'],speed=0.2,wait=True)
    def smalldoor4pocketsidesmall(force,z):
        # Load configuration from YAML
        config = load_config()
    ...
    if ylen == "null":
        ...
        if ylen > 600:
            smalldoor4pocketsidebig(force,z)
        else:
            smalldoor4pocketsidesmall(force,z)
    else:
        print(f"Invalid ylen value type: {type(ylen)} - expected number or 'null'")

if __name__ == "__main__":
    smalldoor1pocket(force=2,z=-1)
