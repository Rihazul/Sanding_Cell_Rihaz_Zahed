...
from model5cycle.outpocketbig import model5outpocketbig
...
def model5outpocketsrun(force,cps):
    def model5outpocketsmall(force,cps):
        ...
        p5 = [p1[0] - outeroffset, p1[1] + outeroffset, z, 180, 0, 0]
        p6 = [p2[0] - outeroffset, p2[1] - outeroffset, z, 180, 0, 0]
        p7 = [p3[0] + outeroffset, p3[1] - outeroffset, z, 180, 0, 0]
        p8 = [p4[0] + outeroffset, p4[1] + outeroffset, z, 180, 0, 0]
        ...
        #Middle Cycle
        communicate(cps=cps,config=config,point=hbpoint1st,tcp=config['coords']['tcpReal'],ucs=config['coords']['ucsTable2'],seventh=-1,speed=speeed,wait=True)
        communicate(cps=cps,config=config,seventh=mx,tcp=config['coords']['tcpReal'],ucs=config['coords']['ucsTable2'],speed=speeed,wait=True)
        perform_process_middle(cps, config, points1=middlepoints,force=force)
    ...
    if xlen == "null":
        ...
        if xlen > 1000:
            model5outpocketbig(force,cps)
        else:
            model5outpocketsmall(force,cps)
    else:
