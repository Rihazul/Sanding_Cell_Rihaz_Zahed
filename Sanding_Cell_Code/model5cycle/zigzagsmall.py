...
from model5cycle.zigzagbig import mod5zigzagbig
...
def load_json_config():
    ...
    return config
def model5zigzagsmalldoor1(force,innerSandingOffset,cps):
    ...
    zigzag_pathp1,prepointp1= generate_zigzag_path(x_coords=x_coords1, y_coords=y_coords1, z_coords=z_coords1, innerOffset=2.5,innerOffsetX=5,innerSandingOffset=innerSandingOffset)
...
def model5zigzagsmalldoor2(force,innerSandingOffset,cps):
    ...
    communicate(cps=cps,config=config,seventh=0,tcp=config['coords']['tcpReal'],ucs=config['coords']['ucsTable2'],speed=0.8,wait=True) 
def mod5zigzagrun(force,innerSandingOffset,cps): 
    def mod5zigzagsmall(force,innerSandingOffset,cps):
        config = load_config()
        ...
        model5zigzagsmalldoor1(force,innerSandingOffset,cps)
        time.sleep(0.5)
        model5zigzagsmalldoor2(force,innerSandingOffset,cps)
    ...
    if xlen == "null":
        ...
        if xlen > 1000:
            mod5zigzagbig(force,innerSandingOffset,cps)
        else:
            mod5zigzagsmall(force,innerSandingOffset,cps)
    else:
        print(f"Invalid ylen value type: {type(xlen)} - expected number or 'null'")
    
if __name__ == "__main__":
    mod5zigzagrun(force=5,innerSandingOffset=50)